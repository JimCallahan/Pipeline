package com.sony.scea.pipeline.plugin.MayaAnimInstanceBuildAction.v1_0_0;
...
public 
class MayaAnimInstanceBuildAction 
extends MayaActionUtils
{
	public
	MayaAnimInstanceBuildAction() 
	{
		super("MayaAnimInstanceBuild", new VersionID("1.0.0"), "SCEA",
				"Extends the MayaBuild Action to now allow for the referencing of Maya files that only " + 
	...
	throws PipelineException
	{
		...
		TreeMap<String,Integer> modelInstances = new TreeMap<String,Integer>();
		TreeMap<String,Integer> modelStarts = new TreeMap<String,Integer>();
				...
				if(fseq.isSingle() && (suffix != null)) {
					if(suffix.equals("ma") || suffix.equals("mb")) {
						...
						if (type.equals("Model"))
						{
							...
							String paddedSpace = nspace;

							for (int each = 0; each < number; each++) {
								if (instanceStart != -1) {
									int current = instanceStart + each;
									String padded = pad(current);
									paddedSpace = nspace + padded;
								}
								reverseNameSpaces.put(paddedSpace, sname);
							}

							modelInstances.put(sname, number);
							modelStarts.put(sname, instanceStart);
						}
						else {
		...
		try {      
			...
			for(String sname : all.keySet()) {
				...
				Integer num = modelInstances.get(sname);
				Integer start = modelStarts.get(sname);
			...
			for(String sname : animPaths.keySet()) {
				...
				out.write("//namespace = " + nspace + "\n");
				out.write("//otherspace = " + otherNspace + "\n");
				out.write("//model name = " + modelName + "\n");
				if (modelName != null) {
					boolean useNSpace = usesNameSpace.get(sname);  
	...
	private static final long serialVersionUID = -6818700517089823752L;
}
