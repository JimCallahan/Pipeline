  ...
  extends BaseTrackingBuilder 
{
    ...
    extends ConstructPass
  {
	...
	{
	  if(!nodeExists(trackNodeName)) 
	    throw new PipelineException
	      ("Somehow the required camera/model tracking data node " + 
	       "(" + trackNodeName + ") does not exist!"); 

	  try {
	    NodeVersion vsn = pClient.getCheckedInVersion(trackNodeName, null); 
	    if(!vsn.hasSources()) {
	      /* lock it if it was generated by an outsourcer */ 
	      lockLatest(trackNodeName); 
	    }
	    else {
	    ...
	    try {
	      ...
	      pClient.getWorkingVersion(getAuthor(), getView(), trackNodeName); 
	    }
	    catch(PipelineException ex2) {
	      throw new PipelineException
		("Somehow no working version of the required camera/model tracking data " + 
		 "node (" + trackNodeName + ") exists in the current working area " + 
	...
	String verifyNodeName = pShotNamer.getTrackingVerifyNode(); 
	{
	  BuildTrackingVerifyStage stage = 
	    ...
	    (pStageInfo, pContext, pClient, 
	     verifyNodeName, trackNodeName, pRorschachVerifyModelNodeName, 
