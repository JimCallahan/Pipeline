package com.intelligentcreatures.pipeline.plugin.SlateSubstAction.v1_5_0; 
...
*    ClientShotName <BR>
*   <DIV style="margin-left: 40px;">
*     Name of the shot.  To be substituted for all instances of "@IC_CLIENT_VERSION@" in the
...
*    LensInfo <BR>
*   <DIV style="margin-left: 40px;">
*     Lens used in the shot.  To be substituted for all instances of "@LENSINFO@" in the
*     template Nuke script.
*   </DIV>
*   <P>
*
*    TakeInfo <BR>
*   <DIV style="margin-left: 40px;">
*     To be substituted for all instances of "@TAKEINFO@" in the
*     template Nuke script.
*   </DIV>
*   <P>
 *
...
{  
  ...
  SlateSubstAction() 
  {
    super("SlateSubst", new VersionID("1.5.0"), "ICVFX",
...

    {
      ActionParam param =
        new StringActionParam
        (aClientShotName,
         "The filename of sequence to be delivered to client as per client's " + 
         "specifications.  This value will be substituted for all instances of " + 
         "\"@IC_FILENAME@\" in the template Nuke script.",
         null);
      addSingleParam(param);
    }

    {
      ActionParam param =
        ...
        (aLensInfo,
         ...
         "This value will be substituted for all instances of \"@LENSINFO@\" in " +
         "the template Nuke script.",
         null);
      addSingleParam(param);
    }

    {
      ActionParam param =
        new StringActionParam
        (aTakeInfo,
         "This value will be substituted for all instances of \"@TAKEINFO@\".",
         null);
      addSingleParam(param);
    }
    ...
    {  
      ...
      layout.addEntry(aClientShotName);
      layout.addEntry(aLensInfo);
      layout.addEntry(aTakeInfo);
      layout.addSeparator();
  ...
  { 
    ...
    String clientShotName = getSingleStringParamValue(aClientShotName); 
    if(clientShotName == null) 
      clientShotName = "";

    String lensInfo = getSingleStringParamValue(aLensInfo);
    if(lensInfo == null)
      lensInfo = "";

    String takeInfo = getSingleStringParamValue(aTakeInfo);
    if(takeInfo == null)
      takeInfo = "";
	...
	while(true) {
	  ...
	  line = sClientShotName.matcher(line).replaceAll(clientShotName);
	  line = sLensInfo.matcher(line).replaceAll(lensInfo);
	  line = sTakeInfo.matcher(line).replaceAll(takeInfo);
  ...
  private static Pattern sClientShotName = Pattern.compile("@IC_SHOTNAME@");
  private static Pattern sLensInfo       = Pattern.compile("@LENSINFO@");
  private static Pattern sTakeInfo       = Pattern.compile("@TAKEINFO@");
