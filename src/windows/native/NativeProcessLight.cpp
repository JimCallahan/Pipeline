// $Id: NativeProcessLight.cpp,v 1.4 2007/02/11 10:30:33 jim Exp $
 ...
 )
{
  ...
  /* get the STDIN file descriptor and HANDLE */ 
 ...
 )
{
  ...
  if(proc == NULL) {
    printf(msg, "unable to open handle for process (%d)", pid);
  ...
  case 9:  /* SIGKILL */
    if(!TerminateProcess(proc, -1)) {
      sprintf(msg, "failed send signal (%d) to process (%d)n", signal, pid);
      env->ThrowNew(IOException, msg);
      return; 
    }
    break;
  ...
  default:
    sprintf(msg, "signal (%d) not supported for Windows processes.\n", pid);
    env->ThrowNew(IOException, msg);
    return;
  }
