  ...
  extends BaseBuilder
{
    ...
    throws PipelineException
  {
    super("ProjectMELBuilder",
          new VersionID("2.3.2"),
          "Temerity", 
          "The Project MEL Builder that works with the basic Temerity Project Names class.",
          mclient,
          qclient,
          builderInformation);
    ...
    
    {
      UtilityParam param = 
        new BooleanUtilityParam
        (aAssetGlobals,
         "Build render globals for asset turntables.", 
         true); 
      addParam(param);
    }
    
    {
      UtilityParam param = 
        new BooleanUtilityParam
        (aAnimGlobals,
         "Build render globals for animation test renders.", 
         true); 
      addParam(param);
    }
...

    {
      ...
      layout.addEntry(2, aAssetGlobals);
      layout.addEntry(2, aAnimGlobals);
      layout.addSeparator(2);
  ...
  protected boolean pAssetGlobals;
  protected boolean pAnimGlobals;
    ...
    extends SetupPass
  {
      ...
      throws PipelineException
    {
      ...
      pAssetGlobals = getBooleanParamValue(new ParamMapping(aAssetGlobals));
      pAnimGlobals = getBooleanParamValue(new ParamMapping(aAnimGlobals));
    ...
    extends ConstructPass
  {
	...
	String script = pProjectNames.getShaderCopyScriptName();
        if (!checkExistance(script)) {
          MELFileStage stage = 
            new MELFileStage(info, pContext, pClient, script, "MayaShdCopyMEL");
          stage.build();
	...
	if (!checkExistance(script)) {
          MELFileStage stage = 
            new MELFileStage(info, pContext, pClient, script, "MayaReRigMEL");
          stage.build();
          addToCheckInList(script);
        }
      }
	...
	if (!checkExistance(script)) {
          MELFileStage stage = 
            new MELFileStage(info, pContext, pClient, script, "MayaRemoveRefMEL");
          stage.build();
          addToCheckInList(script);
        }
      }
	...
	if (!checkExistance(script)) {
          MELFileStage stage =
            new MELFileStage(info, pContext, pClient, script, "MayaMRayInitMEL");
          stage.build();
          addToCheckInList(script);
        }
      }
	...
	if (!checkExistance(script)) {
	  EmptyFileStage stage = new EmptyFileStage(info, pContext, pClient, script, "mel");
	  stage.build();
	...
	for (String type : types) {
	  ...
	  if (!checkExistance(script)) {
	    CatFilesStage stage = 
	      new CatFilesStage(info, pContext, pClient, script, "mel", collectedScripts);
	    stage.build();
	    addToCheckInList(script);
	  }
	}
	...
	for (String script : scripts) {
	  if (!checkExistance(script)) {
	    EmptyFileStage stage = 
	      new EmptyFileStage(info, pContext, pClient, script, "mel");
	    stage.build();
	    pEmptyFiles.add(stage);
	    addToCheckInList(script);
	  }
	...
	if (!checkExistance(script)) {
          MELFileStage stage =
            new MELFileStage(info, pContext, pClient, script, "MayaRenderGlobals");
          stage.build();
          addToCheckInList(script);
	}
      }
