  ...
  implements MouseListener, MouseMotionListener, GLEventListener, KeyListener, ActionListener
{
  ...
  JResourceUsageHistoryDialog() 
  {
	...
	// sorting and statistics type filter toggle buttons here... 
  ...
  )
  {
	...
	for(String hname : pSamples.keySet()) {
	  ...
	  if(block != null) {
	    ...
	    /* system load graphs */ 
	    if((block.getNumSamples() > 0) && (block.getNumProcessors() > 0)) {
		...
		gl.glBegin(gl.GL_QUADS);
		{
		  ...
		  {
		    boolean over = false;
		    gl.glColor3d(0.0f, 0.59f, 1.0f);
		    
		    int wk;
		    for(wk=0; wk<block.getNumSamples(); wk++) {
		      double load = ((double) block.getLoad(wk)) / (procs*1.5);
		      if(load > 0.0) {
			if(load > 1.0) {
			  if(!over) {
			    gl.glColor3d(0.5f, 0.79f, 1.0f);
			    over = true;
			  }
			}
			else if(over) {
