  ...
  implements KeyListener, PopupMenuListener, ActionListener
{
  ...
  {  
	...
	new ViewerNodeHint(this, 
      ...
      pMaxVisibleNodes = 10000L;                             
      pCollapsedNodePaths = new ListPathSet<String>(); 
      pPreviouslyDisplayedRoots = new TreeSet<String>();
  ...
  { 
    ...
    TreeSet<String> newRoots = new TreeSet<String>();
    for(String name : pRoots.keySet()) {
      if(!pPreviouslyDisplayedRoots.contains(name))
        newRoots.add(name);
    }
    pCollapsedNodePaths.removeExceptStartingWith(pRoots.keySet());
    ...
    pPreviouslyDisplayedRoots.clear(); 
    pPreviouslyDisplayedRoots.addAll(pRoots.keySet());
	  ...
	  Toolkit.getDefaultToolkit().beep();
        ...
        UIMaster.getInstance().showErrorDialog
          ("Warning:  Layout Policy Overridden",
           ...
           "during display of the current node networks. All subsequent nodes were " + 
           "therefore displayed in a collapsed mode automatically.\n\n" +
    ...
    /* set the collapsed state of the node */ 
    {
      if((upstream && status.hasSources()) || 
        ...
        if(pNumVisibleNodes > pMaxVisibleNodes) {
          ...
          setNodeCollapsed(path, collapsed);
        }
        else {
          switch(rootPolicy) {
          case Preserve:
            vnode.setCollapsed(wasNodeCollapsed(path));
            break;
          ...
          case CollapseAll:
            vnode.setCollapsed(true);
            setNodeCollapsed(path, true);
            break;
            
          case ExpandAll:
            setNodeCollapsed(path, false);	   
          }
        }
      }
	  ...
	  if((mods & (on1 | off1)) == on1) {
	    if(vunder.getNodeStatus().hasSources()) {
	      vunder.toggleCollapsed();
              setNodeCollapsed(vunder.getNodePath(), vunder.isCollapsed());
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.releaseMasterMgrClient(client);
	  master.endPanelOp(pGroupID, "Roots Found.");
	}
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.releaseMasterMgrClient(client);
	  master.endPanelOp(pGroupID, "Nodes Linked.");
	}
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.releaseMasterMgrClient(client);
	  master.endPanelOp(pGroupID, "Nodes Unlinked.");
	}
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.releaseMasterMgrClient(client);
	  master.endPanelOp(pGroupID, "Secondary Sequence Added.");
	}
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.releaseMasterMgrClient(client);
	  master.endPanelOp(pGroupID, "Secondary Sequence Removed.");
	}
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.releaseMasterMgrClient(client);
	  master.endPanelOp(pGroupID, "Node Properties Exported.");
	}
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.releaseMasterMgrClient(client);
	  master.endPanelOp(pGroupID, "Node Renamed.");
	}
	...
	if(!oname.equals(nname)) {
	  ...
	  boolean wasCollapsed = wasNodeCollapsed(pOldPath);
	  setNodeCollapsed(npath, wasCollapsed); 
	  setNodeCollapsed(pOldPath, false);
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.releaseMasterMgrClient(client);
	  master.endPanelOp(pGroupID, "Node Renumbered.");
	}
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.releaseMasterMgrClient(client);
	  master.endPanelOp(pGroupID, "Nodes Found.");
	}
      }
      ...
      try {
        if(master.beginPanelOp(pGroupID, "Releasing Nodes...")) {
          try {
            ...
            master.endPanelOp(pGroupID, "Nodes Released.");
          }
    ...
    extends Thread
  {
	...
	try {	
	  ...
	  master.releaseMasterMgrClient(client);
	  master.endPanelOp(pGroupID, "Node Deleted.");
	}
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.endPanelOp(pGroupID, "Nodes Checked-In.");
          client.removeMonitor(monitorID); 
	  master.releaseMasterMgrClient(client);
	}
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.endPanelOp(pGroupID, "Nodes Checked-Out.");
          client.removeMonitor(monitorID); 
	  master.releaseMasterMgrClient(client);
	}
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.releaseMasterMgrClient(client);
	  master.endPanelOp(pGroupID, "Nodes Locked.");
	}
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.endPanelOp(pGroupID, "Node Bundle Packed.");
          client.removeMonitor(monitorID); 
	  master.releaseMasterMgrClient(client);
	}
    ...
    extends Thread
  {
    ...
    run() 
    {
 	...
 	try {
 	  ...
 	  master.endPanelOp(pGroupID, "Node Bundle Unpacked.");
          client.removeMonitor(monitorID); 
 	  master.releaseMasterMgrClient(client);
 	}
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.releaseMasterMgrClient(client);
	  master.endPanelOp(pGroupID, "Found Offline Versions.");
	}
    ...
    extends Thread
  {
	...
	try {
	  ...
	  master.releaseMasterMgrClient(client);
	  master.endPanelOp(pGroupID, "Restore Requested.");
	}
      }
    }
   ...
   */ 
  private TreeSet<String> pPreviouslyDisplayedRoots;  
