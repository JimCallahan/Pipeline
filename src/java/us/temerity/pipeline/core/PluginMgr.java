...
{  
   ...
   *   plplugin-db.<I>YYMMDD</I>.<I>HHMMSS</I>.tar<P>
   * </DIV>
   * 
  ...
  backupDatabase
  ( 
   PluginBackupDatabaseReq req
  )
  {
    Path targetDir = req.getBackupDirectory();

    String dateStr = req.getDateString();
    Path backupTarget = new Path(targetDir, "plplugin-db." + dateStr + ".tar");
      ...
      try {
        ...
        SubProcessLight proc = 
          new SubProcessLight("DatabaseBackupArchive", "tar", args, System.getenv(), 
                              PackageInfo.sPluginMgrPath.getParentPath().toFile());
        try {
          proc.start();
          proc.join();
        ...
        LogMgr.getInstance().logStage(LogMgr.Kind.Bak, LogMgr.Level.Info, timer); 
