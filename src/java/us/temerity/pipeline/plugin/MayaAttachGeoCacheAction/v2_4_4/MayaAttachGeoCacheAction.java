// $Id: MayaAttachGeoCacheAction.java,v 1.1 2008/10/24 17:23:37 jim Exp $
 ...
 *   Script Node Method
 *   <DIV style="margin-left: 40px;">
 *     A script node is created by this MEL script in the current scene which will insure
 *     that the geometry cache is always loaded.  There are two approaches to this: 
 *     <DIV style="margin-left: 40px;">
 *       Local - The script node loads only this specific cache, but won't work correctly
 *               if the scene modified by this MEL script is referenced into another 
 *               namespace.<BR>
 *       Global - The script node loads all caches regardless of origin.  Works when 
 *                referenced, but is less efficient when multiple caches are involved.
 *     </DIV> 
 *   </DIV> <BR>
 *   
 *   Pre Cache MEL <BR>
...
{  
  ...
  {
    super("MayaAttachGeoCache", new VersionID("2.4.4"), "Temerity",
    ...
    
    {
      ...
      ActionParam param = 
        new EnumActionParam
          (aScriptNodeMethod, 
           "A script node is created by this MEL script in the current scene which will " + 
           "insure that the geometry cache is always loaded.  The Local method is more " + 
           "efficient, but doesn't work if the modified scene is referenced.  The Global " + 
           "method is less efficient, but more portable.", 
           "Local", choices);
      addSingleParam(param);
    }
...

    {
      ...
      layout.addEntry(aCacheChannel);
      layout.addEntry(aGeometryName);
      layout.addEntry(aScriptNodeMethod);
      layout.addSeparator(); 
  ...
  { 
    ...
    /* name of the cache channel to attach */ 
    String cacheChannel = getSingleStringParamValue(aCacheChannel); 
    ...
    try {
      ...
      String method = getSingleStringParamValue(aScriptNodeMethod); 
      if(method.equals("Local")) {
        out.write
          ("  string $cacheScript = " + 
           "(\"getAttr \\\"\" + $cacheNode + \".cachePath\\\"\");\n");
      }
      else if(method.equals("Global")) {
        out.write
          ("  string $cacheScript = " + 
           "(\"{string $caches[] = `ls -type cacheFile`; string $cache; " + 
           "for ($cache in $caches) { getAttr ($cache + \\\".cachePath\\\"); } }\");\n");
      }
      else {
        throw new PipelineException
          ("Unknown " + aScriptNodeMethod + " (" + method + ") specified!");
      }
  ...
  private static final long serialVersionUID = 8707498167340983637L;
  
  public static final String aCacheFile        = "CacheFile";
  public static final String aCacheChannel     = "CacheChannel";
  public static final String aGeometryName     = "GeometryName";
  public static final String aScriptNodeMethod = "ScriptNodeMethod"; 
