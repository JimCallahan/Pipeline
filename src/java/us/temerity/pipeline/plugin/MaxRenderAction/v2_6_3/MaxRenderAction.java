...
package us.temerity.pipeline.plugin.MaxRenderAction.v2_6_3;
...
{  
  ...
  MaxRenderAction() 
  {
    super("MaxRender", new VersionID("2.6.3"), "Temerity",
  ...
  { 
    ...
    Path preRenderScript = null;
    try {
      File script = createTemp(agenda, "ms");
      preRenderScript = new Path(script);

      FileWriter out = new FileWriter(script);       

      String sceneName = getSingleStringParamValue(aMaxScene, false);
      FileSeq sourceSeq = agenda.getPrimarySource(sceneName);
      Path scenesPath = sourceScene.getParentPath();
      out.write(getProjectInitScript(new FileSeq(scenesPath.toString(), sourceSeq)) + "\n");
         
      /* if there is a pre-render script, copy its contents */ 
      Path preScript = getMaxScriptSourcePath(aPreRenderScript, agenda);
      if(preScript != null) {
        out.write("\n" + 
                  "/*\n" + 
                  " * Copied from: " + preScript.toOsString() + "\n" +
                  " */\n\n");

        FileReader in = new FileReader(preScript.toFile()); 
        char cs[] = new char[1024];
        while(true) {
          int len = in.read(cs, 0, 1024);
          if(len == -1) 
            break;
      ...
      throw new PipelineException
        ("Unable to create temporary pre-render MAXScript file (" + preRenderScript + ") " + 
         "required by the " + getName() + " Action to setup project paths!\n" + 
           ex.getMessage());
    }
  ...
  private static final long serialVersionUID = 6410938542902564418L;
