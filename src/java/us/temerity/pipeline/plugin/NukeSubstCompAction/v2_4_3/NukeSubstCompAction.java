// $Id: NukeSubstCompAction.java,v 1.1 2008/03/30 22:09:48 jim Exp $
 ...
 * file sequence of the node using this action (Process or Substitute Mode).  If a Nuke Write
 * node is encountered which uses the string "@PL_TARGET@" as its file option, this will
 * be replaced with the primary target image sequence associated with this node (Process 
 * Mode).  Alternatively, this action can append a Write node to the dynamically generated 
 * script which will write the primary target file sequence images (Append & Process Mode).  
 * In all cases, this action only supports a single target file sequence.  See the NukeComp 
 * action for an alternative way of executing Nuke scripts that does support multiple target 
 * image sequences.<P> 
 * 
 ...
 *     generates the primary target image sequence for the node.  Tf a Nuke Write node is 
 *     encountered which uses the string "@PL_TARGET@" as its file option, this will be 
 *     replaced with the full path to the actual primary target image sequence for the 
 *     node.<P> 
...
{  
  ...
  NukeSubstCompAction() 
  {
    super("NukeSubstComp", new VersionID("2.4.3"), "Temerity",
          ...
          while(true) {
            ...
            /* we are in the middle of processing a node block... */ 
              ...
              /* or is this the "file" property of a Read or Write node? */ 
                ...
                else if(!mode.equals(aSubstitute)) {
                  isDisabled = true;
                }
              }
              
              if(!replaceLine) {
                if(line.equals("}") && isDisabled) 
                  buf.append(" disable true\n");
                buf.append(line + "\n"); 
              }
                  ...
                  try {
                    ...
                    while(true) {
                      ...
                      /* we are in the middle of processing a node block... */ 
                        ...
                        if(!rreplaceLine) {
                          if(rline.equals("}") && risDisabled) 
                            rbuf.append(" disable true\n");
                          rbuf.append(rline + "\n"); 
                        }
    ...
    throws PipelineException 
  {
    ...
    if(typeName.equals("Write") && 
       (file.equals("@PL_TARGET@") || file.endsWith(":@PL_TARGET@"))) {
      if(nukePat != null) 
        return (" file ./" + nukePat + "\n"); 
      else 
        return null; 
    ...
    if(typeName.equals("Write")) {
      ...
      if((nukePat != null) && nukePat.equals(npat)) {
        return (" file ./" + npat + "\n"); 
      }
    }
  ...
  private static final long serialVersionUID = 8068631768184378027L;
