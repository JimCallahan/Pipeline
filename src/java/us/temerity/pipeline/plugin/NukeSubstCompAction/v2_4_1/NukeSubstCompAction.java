 ...
 * Nuke script with Nuke script fragments provided by other source scripts and then executing 
 * the resulting script with Nuke.<P> 
 * 
 ...
 *   Append Write <BR>
 *   <DIV style="margin-left: 40px;">
 *     Whether to append a Nuke Write node to the end of dynamically generated Nuke script
 *     which will write the primary target image sequence when executed. 
 *   </DIV> <BR>
 * 
 *   Extra Options <BR>
 *   <DIV style="margin-left: 40px;">
...
{  
  ...
  NukeSubstCompAction() 
  {
    super("NukeSubstComp", new VersionID("2.4.1"), "Temerity",
          "Dynamically assemples a Nuke script by replacing specific Nuke nodes in a " + 
          "master source Nuke script with Nuke script fragments provided by other source " + 
          "scripts and then executing the resulting script with Nuke."); 
...

    {
      ActionParam param = 
        ...
        (aAppendWrite,
         "Whether to append a Nuke Write node to the end of dynamically generated Nuke " + 
         "script which will write the primary target image sequence when executed.", 
         true);
      addSingleParam(param);
    }
  ...
  { 
    ...
    String targetNukePattern = null; 
    {
      if(!agenda.getSecondaryTargets().isEmpty()) 
        throw new PipelineException
          ("The NukeSubtComp action does not support secondary target file sequences!"); 
    ...
    if(getSingleBooleanParamValue(aAppendWrite)) {
  ...
  private static final long serialVersionUID = -8519741946724620920L;

  public static final String aMasterScript = "MasterScript"; 
  public static final String aAppendWrite  = "AppendWrite";
