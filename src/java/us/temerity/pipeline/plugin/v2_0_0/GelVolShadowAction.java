// $Id: GelVolShadowAction.java,v 1.2 2006/05/07 21:30:13 jim Exp $
...
/*------------------------------------------------------------------------------------------*/
/*   G E L   V O L   S H A D O W   A C T I O N                                              */
/*------------------------------------------------------------------------------------------*/

/** 
 * Generate optimized Gelato volume shadow maps from volume depth maps. 
 * 
 * Converts the volume depth maps which make up the primary file sequence of one of the source
 * nodes into the volume shadow maps which make up the primary file sequence of this node. <P>
 * 
 ...
 *   Opaque Width <BR>
 *   <DIV style="margin-left: 40px;">
 *     A hint about the depth range over which which the opaque z's within a pixel are 
 *     considered "the same object" when multiple samples per pixel are combined into a 
 *     single output pixel.
 *   </DIV> <BR>
 * 
 *   Format <BR>
...
{  
  ...
  GelVolShadowAction() 
  {
    super("GelVolShadow", new VersionID("2.0.0"), "Temerity",
	  "Generate optimized Gelato volume shadow maps from volume depth maps.");
	...
	("OpaqueWidth",
	 "A hint about the depth range over which which the opaque z's within a pixel are " + 
	 "considered \"the same object\" when multiple samples per pixel are combined " + 
    ...
    throws PipelineException
  {
    ...
    {
      Double width = (Double) getSingleParamValue("OpaqueWidth");
      if(width != null) {
	args.add("-opaquewidth");
  ...
  private static final long serialVersionUID = -3169486318363903636L;
