// $Id: MayaTextureSyncTool.java,v 1.7 2006/11/22 09:08:01 jim Exp $
...
{  
    ...
    throws PipelineException 
  {
	...
	if((path != null) && (path.length() > 0)) {
	  File file = new File(PackageInfo.sWorkDir, PackageInfo.sUser + "/" + pView + path);
	  if(!file.isDirectory()) 
	    throw new PipelineException
  ...
  {    
	...
	try {
	  ...
	  if(tex.startsWith("$WORKING")) 
	    texture = (workRoot + texture.substring(8));
	...
	if(path.startsWith(workRoot)) 
	  pTextureFiles.put(file, new File(path.substring(workRoot.length())));
	else 
  ...
  )  
  {
	...
	String path = dir.getPath();
	if(path.startsWith(rpath)) 
	  field.setText(path.substring(rpath.length()));
      }
    }
  }
  ...
  private static final long serialVersionUID = 2645047202995702095L;
