// $Id: GelShadowCubeAction.java,v 1.1 2007/06/17 15:34:40 jim Exp $
...
/*------------------------------------------------------------------------------------------*/
/*   G E L   S H A D O W   C U B E   A C T I O N                                            */
/*------------------------------------------------------------------------------------------*/

/** 
 * Generate optimized Gelato cubic faced shadow maps from six directional source 
 * depth maps. <P> 
 * 
 * Converts the six depth maps [+x, -x, +y, -y, +z, -z] which make up the primary file 
 * sequence of one of the source nodes into the single cubic shadow map which is the 
 ...
 *   Format <BR>
 *   <DIV style="margin-left: 40px;">
 *     The image format of the generated shadow map:
 *     <UL>
 *       <LI> TIFF 
 *       <LI> OpenEXR
 *     </UL>
 ...
 *   Planar Config <BR>
 *   <DIV style="margin-left: 40px;">
 *     The planar configuration of the output shadow map:
 *     <UL>
 *       <LI> Contiguous
 *       <LI> Seperate
 *     </UL>
...
{  
  ...
  GelShadowCubeAction() 
  {
    super("GelShadowCube", new VersionID("2.0.0"), "Temerity", 
	  "Generate optimized Gelato cubic faced shadow maps from six directional source " +
	  "depth maps.");
	...
	new LinkActionParam
	("DepthSource",
	 "The source node which contains the six depth map files [+x, -x, +y, -y, +z, -z] " + 
	 "to convert.",
	 null);
    ...
    throws PipelineException
  {
	...
	if(!fseq.isSingle() || (suffix == null) || !suffix.equals("gsm"))
	  throw new PipelineException
	    ("The target primary file sequence (" + fseq + ") must contain a " + 
	     "single Gelato shadow map (.gsm)!");
  ...
  private static final long serialVersionUID = 7475369933451670759L;
