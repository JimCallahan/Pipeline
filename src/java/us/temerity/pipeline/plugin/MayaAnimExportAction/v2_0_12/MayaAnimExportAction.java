// $Id: MayaAnimExportAction.java,v 1.1 2007/06/19 18:24:49 jim Exp $
...
{  
  ...
  {
    super("MayaAnimExport", new VersionID("2.0.12"), "Temerity", 
    ...
    throws PipelineException
  {
    ...
    /* sanity checks */ 
    Path touch = null;
    TreeMap<String,Path> targetAnims = new TreeMap<String,Path>();
    String method = null;
	...
	if(suffix == null) 
	  touch = fseq.getPath(0);
	else if(suffix.equals("anim")) 
	  targetAnims.put(fpat.getPrefix(), fseq.getPath(0));
	else 
	  throw new PipelineException
	...
	if(sname != null) {
	  ...
	  NodeID snodeID = new NodeID(nodeID, sname);
	  prepMel = new Path(PackageInfo.sProdPath,
			     snodeID.getWorkingParent() + "/" + fseq.getPath(0));
	}
      }
	...
	if(sname != null) {
	  ...
	  NodeID snodeID = new NodeID(nodeID, sname);
	  exportMel = new Path(PackageInfo.sProdPath,
			       snodeID.getWorkingParent() + "/" + fseq.getPath(0));
	}
	...
	for(String prefix : targetAnims.keySet()) {
	  Path target = targetAnims.get(prefix);
	  out.write
	    ("{" + 
	      "// EXPORT: " + prefix + "\n" +
	...
	(agenda.getNodeID().getAuthor(), getName() + "-" + agenda.getJobID(), 
	 program, args, senv, agenda.getWorkingDir(), 
	 outFile, errFile);
    }
    catch(Exception ex) {
      throw new PipelineException
  ...
  private static final long serialVersionUID = -2255726263684304132L;
