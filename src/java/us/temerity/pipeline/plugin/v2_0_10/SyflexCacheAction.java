...
public class SyflexCacheAction extends BaseAction {

	public SyflexCacheAction() {
		super("SyflexCache", new VersionID("2.0.10"), "Temerity",
				"Creates a syflex cache");


		{
			ActionParam param = new LinkActionParam(aMayaScene,
					"The source Maya scene node.", null);
			addSingleParam(param);
		}

		{
			ActionParam param = new StringActionParam(aClothObject,
					"The name of the piece of cloth to be simulated.", null);
			addSingleParam(param);
		}
	}
	...
	/**
	 * Construct a {@link SubProcessHeavy SubProcessHeavy} instance which when
	 * executed will fulfill the given action agenda.
	 * <P>
	 * 
	 * @param agenda
	 ...
	 *             If unable to prepare a SubProcess due to illegal, missing or
	 *             imcompatable information in the action agenda or a general
	 *             failure of the prep method code.
	 */
	public SubProcessHeavy prep(ActionAgenda agenda, File outFile, File errFile)
		...
		try {
			...
			out.println("string $childs[] = `listRelatives -shapes "
					+ clothName + "`;");
			out.println("string $child = $childs[0];");
			out.println("int $startFrame = " + cacheStart + ";");
			out.println("currentTime ($startFrame - 1);");
			...
			out.println("setAttr -type \"string\" ($envNode + \".envPath\") \""
					+ cachePath + "\";");
			out
					.println("string $expandedName = `getAttr ($child + \".cacheName\")`;");
			out.println("system (\"rm \" + $expandedName + \".*\");");
			out
					.println("connectAttr -f ($envNode + \".absPath\") ($child + \".cacheName\");");
			out.println("");
			out.println("int $frame;");
			out
					.println("for ($frame = $startFrame -1;  $frame <= $endFrame + 1; $frame++)");
			out.println("{");
			out.println("		 float $sy_timer = `timerX`;");
			out.println("		 currentTime -e $frame;");
			out
					.println(" 		 string $shape[] = `listConnections -sh 1 ($child + \".outMesh\")`;");
			out.println("		 int $place = 0;");
			out.println(" 		 while (`nodeType $shape[$place]` != \"mesh\")");
			out.println("		 {");
			out.println("		 		 string $node = $shape[0];");
			out
					.println("		 		 if (attributeExists(\"outputGeometry\", $node))");
			out
					.println("		 		 		 $shape = `listConnections -sh 1 ($node+\".outputGeometry\")`;");
			out.println("		 		 else");
			out.println("		 		 		 $place++;");
			out.println("		 }");
			...
			out
					.println("		 print (\"frame \" + $frame + \" took syflex \" + ($sy_roundTime) + \" seconds.\\n\");");
			out.println("}");
	...
	private static final long serialVersionUID = 3479669817910762646L;
}