...
{  
  ...
  ShakeCompAction() 
  {
    super("ShakeComp", new VersionID("2.0.9"), "Temerity",
  ...
  { 
	...
	if(fseq == null) 
	  throw new PipelineException
	    ("Somehow the Shake Script node (" + sname + ") was not one of the " + 
	     "source nodes!");
	...
	if(!fseq.isSingle() || 
	  ...
	  throw new PipelineException
	    ("The Shake Comp Action requires that the source node specified by the Shake " +
	     "Script parameter (" + sname + ") must have a single Shake script file " + 
	     "(.shk) as its primary file sequence!");
  ...
  private static final long serialVersionUID = 5017192069834380395L;
