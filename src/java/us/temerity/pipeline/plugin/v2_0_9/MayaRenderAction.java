// $Id: MayaRenderAction.java,v 1.1 2006/05/26 17:06:35 jim Exp $
...
{  
  ...
  MayaRenderAction() 
  {
    super("MayaRender", new VersionID("2.0.9"), "Temerity",
  ...
  { 
	...
	if(sname != null) {
	  ...
	  NodeID snodeID = new NodeID(nodeID, sname);
	  preRender = new Path(PackageInfo.sProdPath,
			       snodeID.getWorkingParent() + "/" + fseq.getPath(0));
	}
      }
	...
	if(sname != null) {
	  ...
	  NodeID snodeID = new NodeID(nodeID, sname);
	  postRender = new Path(PackageInfo.sProdPath,
				snodeID.getWorkingParent() + "/" + fseq.getPath(0));
	}
      }
	...
	if(sname != null) {
	  ...
	  NodeID snodeID = new NodeID(nodeID, sname);
	  preFrame = new Path(PackageInfo.sProdPath,
			      snodeID.getWorkingParent() + "/" + fseq.getPath(0));
	}
      }
	...
	if(sname != null) {
	  ...
	  NodeID snodeID = new NodeID(nodeID, sname);
	  postFrame = new Path(PackageInfo.sProdPath,
			       snodeID.getWorkingParent() + "/" + fseq.getPath(0));
	}
      }
    }
	...
	throw new PipelineException
      ...
      Path renderPath = new Path(PackageInfo.sProdPath, 
	...
	program = (program + ".exe");
 	...
 	(agenda.getNodeID().getAuthor(), getName() + "-" + agenda.getJobID(), 
 	 program, args, env, agenda.getWorkingDir(), 
 	 outFile, errFile);
    }
    catch(Exception ex) {
      throw new PipelineException
  ...
  private static final long serialVersionUID = 5098527449772225351L;
