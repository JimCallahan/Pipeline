// $Id: MayaBuildAction.java,v 1.1 2007/06/19 18:24:49 jim Exp $
...
/*----------------------------------------------------------------------------------------*/
  ...
  MayaBuildAction() 
  {
    super("MayaBuild", new VersionID("2.0.11"), "Temerity",
    ...
    
    {
      ...
      layout.addEntry(PARAM_linear);
      layout.addEntry(PARAM_angular);
      layout.addEntry(PARAM_time);
      layout.addSeparator();
  ...
  getInitialSourceParams()
  {
	...
	(PARAM_useNameSpace,
	 "Should Maya use the namespace flag to do the import. Highly recomended to turn this on.",
	 true);
      params.put(param.getName(), param);
    }
	...
	new EnumActionParam
	(PARAM_type,
	 "Should Maya Import or Reference the source.", "Reference", 
	 choices);  	 
      params.put(param.getName(), param);
    }
    ...
    throws PipelineException
  {
	...
	saveScene = new Path(PackageInfo.sProdPath,
      ...
      {
	linearUnit = (String) getSingleParamValue(PARAM_linear);
	if(linearUnit == null)
	  throw new PipelineException 
      ...
      {
	angularIndex = ((EnumActionParam) getSingleParam(PARAM_angular)).getIndex() ;
	if (angularIndex < 0)
	  throw new PipelineException 
      ...
      {
	timeIndex = ((EnumActionParam) getSingleParam(PARAM_time)).getIndex() ;
	if (angularIndex < 0)
	  throw new PipelineException 
	...
	String type = (String) getSourceParamValue(sname, PARAM_type);
	boolean namespace = (Boolean) getSourceParamValue(sname, PARAM_useNameSpace);
  ...
  private static final String PARAM_linear = "LinearUnits";
  private static final String PARAM_angular = "AngularUnits";
  private static final String PARAM_time = "TimeUnits";
  private static final String PARAM_useNameSpace = "NameSpace";
  private static final String PARAM_type = "BuildType";
