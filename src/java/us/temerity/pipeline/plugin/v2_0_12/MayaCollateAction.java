// $Id: MayaCollateAction.java,v 1.1 2006/07/07 00:07:53 jim Exp $
...
{  
  ...
  MayaCollateAction() 
  {
    super("MayaCollate", new VersionID("2.0.12"), "Temerity", 
	...
	for(Integer order : anims.keySet()) {
	  ...
	  for(String prefix : animPaths.keySet()) {
	    ...
	    /* copy the animation from the WORKING model to FINAL model */ 
	    {
	      ...
	      out.write
		("select -r \"" + loadSet + "\";\n" +
		 "string $loadList[] = `ls -sl`;\n" + 
		 "select -r \"" + applySet + "\";\n" +
		 "string $list[] = `ls -sl`;\n" +
		 "int $value = arrayCompare($list, $loadList);\n" +
		 "if ($value)\n" +
		 "{\n" +
		 "	select -r \""+ loadSet +"\";\n" +
		 "} else\n" +
		 "{\n" +
		 "	string $newList[] = sort($loadList);\n" +
		 "	select -r $newList;\n" +
		 "}\n" +
		 "copyKey\n" +
		 "  -clipboard anim\n" +
  ...
  /**
   * writes out the removePrefixLong and the arrayCompare methods.
   * 
   * @param out
   * 	The FileWriter to print the mel to
   * @throws IOException
   */
  private void writeHelperProcs(FileWriter out) throws IOException
  {
    out.write("global proc string removePrefixLong(string $name)\n" +
      	      "{\n" +
      	      "\tstring $toReturn;\n" + 
      	      "\tstring $buffer[];\n" + 
	      ...
	      "}\n" + 
	      "\n\n\n" + 
	      "global proc int arrayCompare(string $a[], string $b[])\n" + 
	      "{\n" + 
	      "\tif (size($a) != size($b))\n" + 
	      "\t\treturn 0;\n" +
	      "\tint $val = size($a);\n" +
      	      ...
      	      "\tfor ($i = 0; $i < $val; $i++)\n" + 
      	      "\t{\n" + 
      	      "\t\tif (removePrefixLong($a[$i]) != removePrefixLong($b[$i]))\n" + 
      	      "\t\t\treturn 0;\n" + 
      	      "\t}\n" + 
      	      "\treturn 1;\n" + 
      	      "}\n" + 
  ...
  private static final long serialVersionUID = -5797305171013932439L;
