// $Id: MayaShaderExportAction.java,v 1.1 2007/06/19 18:24:51 jim Exp $
...
/*----------------------------------------------------------------------------------------*/
  ...
  {
    super("MayaShaderExport", new VersionID("2.0.12"), "Temerity",
	  "Exports all shaders with a given prefix.");
  ...
  { 
    ...
    String prefixName = (String) getSingleParamValue(aSelectionPrefix);
	...
	out.println();
      ...
      out.println("string $filter1 = `itemFilterRender -shaders 1`;");
      out.println("string $filter2 = `itemFilterRender -anyTextures 1`;");
      out.println("string $filter3 = `itemFilterRender -renderUtilityNode 1`;");
      out.println("string $one[] = `lsThroughFilter -na $filter1 -sort byName -reverse false`;");
      out.println("string $two[] = `lsThroughFilter -na $filter2 -sort byName -reverse false`;");
      out.println("string $three[] = `lsThroughFilter -na $filter3 -sort byName -reverse false`;");
      out.println("select -r -ne $one $two $three;");
      if (prefixName != null)
	out.println("string $mats[] = `ls -sl \"" + prefixName + "*\"`;");
      else
	out.println("string $mats[] = `ls -sl`;");
      out.println("if (size($mats) > 0 )");
      out.println("{");
	...
	output += " -typ \"mayaBinary\"";
      ...
      out.println("file -rename \"" + scene.toOsString() + "\";");
      out.println("file -type \"" + (isAscii ? "mayaAscii" : "mayaBinary") + "\";");
  ...
  private static final long serialVersionUID = -3720492818927404238L;
