 ...
 * 
 */
public class BuildMattePassTool extends BaseTool{

	/*-----------------------------------------------*/
	/*             STATIC INTERNAL VARS              */
	/*-----------------------------------------------*/
	private static final long serialVersionUID = -2467057972166485954L;	
...
//	private final String basicCleanMEL = "/projects/lr/assets/tools/render/sceaBasicClean";
	 ...
	 */
	private final String rndrGlob = rndrPattern + "settings-matte";
	
	/**
	 *  RegExp that matches lighting shots. 
	 **/
	private final String mattePattern = ".*/production/.*/lgt/.*_matte";
	 ...
	 */
	public BuildMattePassTool()
	{
		super("BuildMattePass", new VersionID("1.0.0"), "SCEA",
		"Builds the render tree for a selected lighting node.");
	...
	public synchronized String collectPhaseInput() throws PipelineException {
		...
		if(!pPrimary.matches(mattePattern))
		...
		if(pLayerNames.isEmpty()){
			System.err.println("i found nothing...");
			return null;
		}
		//pLayerNames.add("z");
		...
		return ": most errors tend to happen between keyboard and chair";
				...
				if(curLine.startsWith("createNode objectSet")) {
					...
					while(strTok.hasMoreTokens()){
						...
						if(token.equals("-n")){
							...
							if(ref.matches("SKY[1-9]")){
								pLayerNames.add(("sky" + num.toString()));
								num++;
							}
						}
					}//end while
...
//					catAction.setSourceParamValue(cleanAllMEL, "Order", 200);
	...
	{		
		...
		for(String pass: curPasses){
		   ...
		   */
			String name = folder + pass + "/" + filePrefix + "_" + pass;
			...
			NodeMod nMod = registerSequence(mclient, name, pad, extension, editor, startFrame, endFrame, byFrame);
			...
			NodeMod tempMelMod = registerNode(mclient, name+"_hide", "mel", melEditor);
			VersionID ver = plugs.get("SCEA", "MayaSelectiveHide").last();
			BaseAction action = plug.newAction("MayaSelectiveHide", ver, "SCEA");
			action.setSingleParamValue("SetName", pass.toUpperCase());
			tempMelMod.setAction(action);
			mclient.modifyProperties(pUser, pView, tempMelMod);
			
			NodeMod rndrMelMod = registerNode(mclient, name+"_rndr", "mel", melEditor);
			VersionID catVer = plugs.get("Temerity", "CatFiles").last();
			
			mclient.link(pUser, pView, rndrMelMod.getName(), tempMelMod.getName(), DEP, LINKALL, null);
			mclient.link(pUser, pView, rndrMelMod.getName(), rndrGlob, DEP, LINKALL, null);
			
			BaseAction catAction = plug.newAction("CatFiles", catVer, "Temerity");
			catAction.initSourceParams(rndrGlob);
			catAction.setSourceParamValue(rndrGlob, "Order", 100);				
			catAction.initSourceParams(tempMelMod.getName());
			catAction.setSourceParamValue(tempMelMod.getName(), "Order", 150);
			rndrMelMod.setAction(catAction);
			mclient.modifyProperties(pUser, pView, rndrMelMod);
			
			//NodeMod rndrMod = registerNode(mclient, name+"-rndr", "mel", melEditor);
			mclient.link(pUser, pView, nMod.getName(), rndrMelMod.getName(), DEP, LINKALL, null);
			mclient.link(pUser, pView, nMod.getName(), mod.getName(), DEP, LINKALL, null);
			
			VersionID rndrVer = plugs.get("Temerity", "MayaMRayRender").last();
			BaseAction rndrAction = plug.newAction("MayaMRayRender", rndrVer, "Temerity");
			rndrAction.setSingleParamValue("MayaScene", mod.getName());
			rndrAction.setSingleParamValue("PreExportMEL", rndrMelMod.getName());
			nMod.setAction(rndrAction);
			mclient.modifyProperties(pUser, pView, nMod);
						
			nMod.setActionEnabled(true);
			mclient.submitJobs(pUser, pView, nMod.getName(), null);
			nMod.setActionEnabled(false);
			
			custRndrMel.put(pass, tempMelMod);
			//custShdrMel.put(pass, shdrMod);				

			pRndrMel.put(pass, tempMelMod.getName());
