// $Id: ExportAnimForRefAction.java,v 1.1 2007/02/13 05:27:17 jesse Exp $
...
public
  ExportAnimForRefAction() 
  {
    super("ExportAnimRef", new VersionID("1.0.0"), "SCEA",
    ...
    
    {
    	...
    	new LinkActionParam
    	(aMayaSrcFile,
    	 "The Maya scene where the animation will be exported from.",
    	 null);
  ...
  public SubProcessHeavy prep(ActionAgenda agenda, File outFile, File errFile) throws PipelineException
  {
    	...
    	
    {
      ...
      */
      ...
      {
    	  String srcMayaName = (String) getSingleParamValue(aMayaSrcFile);
    	  if (srcMayaName == null)
    		  throw new PipelineException("Valid Maya file is required!");
    	  
    	  FileSeq fseq = agenda.getPrimarySource(srcMayaName);
    	  String suffix = fseq.getFilePattern().getSuffix();
    	  if(fseq.isSingle() && (suffix != null))
    	  {
    		  if(suffix.equals("ma") || suffix.equals("mb"))
    		  {
    			  //get full path...
    			  NodeID sNodeID = new NodeID(agenda.getNodeID(), srcMayaName);
    			  srcMayaFile = new Path(PackageInfo.sProdPath, sNodeID.getWorkingParent() + "/" + fseq.getPath(0)).toString();
    			  
    			  //srcMayaFile = new Path(PackageInfo.sProdPath, agenda.getNodeID().getWorkingParent() + "/" + fseq.getPath(0)).toString();
    		  }
    	  }
    	  if(srcMayaFile == null) 
    		  throw new PipelineException ("Not a valid maya file!");
      }
      ...
      /* the generated Maya scene filename */ 
      {
    	  ...
    	  if(!fseq.isSingle() || (suffix == null) || !(suffix.equals("ma") || suffix.equals("mb")))
    		  throw new PipelineException("The MayaImport Action requires that the primary target file sequence must be a single Maya scene file."); 
    ...
    saveSceneName = saveScene.toString().replace("_make", "_export");
    ...
    try {      
      ...
      //open the maya file...
      out.write("file -o -options \"v=0\" \"" + srcMayaFile + "\";\n\n");
      /*reference in skeleton file with specific namespace*/
      String namespace = "test";
      out.write("select -r `referenceQuery -n " + namespace + "RN`;\n\n");
      //out.write("file -r -type \"mayaBinary\" -gl -namespace \"" + namespace + "\"-options \"v=0\" -pr \"" + srcMayaFile + "\";\n\n");
      ...
      out.write("file -f -type \"mayaAscii\" -esa \"" + saveSceneName + "\";\n\n");
...
*/      
  ...
  /*----------------------------------------------------------------------------------------*/
  private static final long serialVersionUID = -7537797512128158447L;
  
  private static final String aMayaSrcFile = "MayaScene";
