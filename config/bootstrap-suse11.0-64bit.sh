...
then 
  ...
  if [ "$suse_release" != "openSUSE 11.0 (X86-64)" ]
  then
    echo "This configuration should only be run from a openSUSE-11.0 (64-bit) machine!"
    echo "  Found: $suse_release"
    exit 1
  fi
else
  echo "Unable to determine if this is an openSUSE-11.0 (64-bit) machine!"
  exit 1
fi
