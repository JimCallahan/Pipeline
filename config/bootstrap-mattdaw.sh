...
plsrcdir=$HOME/code-mattdaw/src/pipeline
plprofile=$plsrcdir/plconfig/customers/mattdaw/$sitep
...
then
  ...
  rsync -av --exclude-from=$plsrcdir/config/excluded --delete \
    $plsrcdir/ $MAC_HOSTNAME:/Users/$USER/code-mattdaw/src/pipeline

  ssh $MAC_HOSTNAME "source .bash_profile; cd code-mattdaw/build/pipeline; ./bootstrap.sh $sitep"
fi
...
then
  ...
  rsync -av --exclude-from=$plsrcdir/config/excluded --delete \
    $plsrcdir/ $WIN_HOSTNAME:/home/$USER/code-mattdaw/src/pipeline

  ssh $WIN_HOSTNAME "source .bash_profile; cd code-mattdaw/build/pipeline; ./bootstrap.sh $sitep"
fi
