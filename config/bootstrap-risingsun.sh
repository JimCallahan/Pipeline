...
plsrcdir=$HOME/code-risingsun/src/pipeline
plprofile=$plsrcdir/plconfig/customers/risingsun/$sitep
...
then
  ...
  rsync -av --exclude-from=$plsrcdir/config/excluded --delete \
    $plsrcdir/ $MAC_HOSTNAME:/Users/$USER/code-risingsun/src/pipeline

  ssh $MAC_HOSTNAME "source .bash_profile; cd code-risingsun/build/pipeline; ./bootstrap.sh $sitep"
fi
...
then
  ...
  rsync -av --exclude-from=$plsrcdir/config/excluded --delete \
    $plsrcdir/ $WIN_HOSTNAME:/home/$USER/code-risingsun/src/pipeline

  ssh $WIN_HOSTNAME "source .bash_profile; cd code-risingsun/build/pipeline; ./bootstrap.sh $sitep"
fi
