...
then 
  ...
  redhat_arch=`uname -i`
  if [ "$redhat_release" != "CentOS release 4.8 (Final)" -o "$redhat_arch" != "i386" ]
  then
    echo "This configuration should only be run from a Centos 4.8 (32-bit) machine!"
    echo "  Found: $redhat_release"
    exit 1
  fi
else
  echo "Unable to determine if this is an CentOS 4.8 (32-bit) machine!"
  exit 1
fi
