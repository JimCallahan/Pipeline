...
then 
  ...
  redhat_arch=`uname -i`
  if [ "$redhat_release" != "CentOS release 5.8 (Final)" -o "$redhat_arch" != "x86_64" ]
  then
    echo "This configuration should only be run from a Centos 5.8 (64-bit) machine!"
    echo "  Found: $redhat_release"
    exit 1
  fi
else
  echo "Unable to determine if this is an CentOS 5.8 (64-bit) machine!"
  exit 1
fi
