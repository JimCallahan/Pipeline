...
then 
  ...
  if [ "$suse_release" != "openSUSE 10.3 (X86-64)" ]
  then
    echo "This configuration should only be run from a openSUSE-10.3 (64-bit) machine!"
    echo "  Found: $suse_release"
    exit 1
  fi
else
  echo "Unable to determine if this is an openSUSE-10.3 (64-bit) machine!"
  exit 1
fi
...
then
  ...
  #rsync -av --exclude-from=$plsrcdir/config/excluded --delete-excluded \
  #  $plsrcdir/ -e ssh $MAC_HOSTNAME:/Users/$USER/code-$customer/src/pipeline


  time \
  ssh $MAC_HOSTNAME "source .bash_profile; \
                     cd code-$customer/build; \
