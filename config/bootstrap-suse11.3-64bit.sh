...
then 
  ...
  if [ "$suse_release" != "openSUSE 11.3 (x86_64)" ]
  then
    echo "This configuration should only be run from a openSUSE-11.3 (64-bit) machine!"
    echo "  Found: $suse_release"
    exit 1
  fi
else
  echo "Unable to determine if this is an openSUSE-11.3 (64-bit) machine!"
  exit 1
fi
